...
function renderCurrentPage() {
  ...
  if (state.pageId !== "simulator" && window.Project4Simulator?.cleanup) {
    window.Project4Simulator.cleanup();
  }
  const renderers = {
    ...
    simulator: () => window.Project4Simulator?.renderPage?.() || renderLoadingPage("仿真", "加载仿真器。"),
...
function bindForms() {
  ...
  if (state.pageId === "simulator") window.Project4Simulator?.bindPage?.();
