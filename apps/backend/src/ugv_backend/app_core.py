...
from . import autopilot as autopilot_helpers
from . import iot as iot_helpers
from . import robot_control as robot_control_helpers
from .config import (
    BASE_DIR,
    DEFAULT_SITE,
    PAGES,
...
app.mount("/static/uploads", StaticFiles(directory=str(UPLOAD_ROOT_DIR)), name="uploads")
...
async def simulator_page(request: Request):
    return render_page(request, "simulator")
...
async def api_robot_camera_snapshot(robot_id: int, request: Request) -> Response:
    ...
    if fallback:
        file_path_value = str(fallback["file_path"]).lstrip("/")
        if file_path_value.startswith("static/uploads/"):
            file_path = UPLOAD_ROOT_DIR / file_path_value[len("static/uploads/") :]
        elif file_path_value.startswith("static/"):
            file_path = STATIC_DIR / file_path_value[len("static/") :]
        else:
            file_path = STATIC_DIR / file_path_value
        if file_path.exists():
            return FileResponse(
                path=str(file_path),
...
UPLOAD_DIR = UPLOAD_ROOT_DIR / "devices"
SENSOR_UPLOAD_DIR = UPLOAD_ROOT_DIR / "sensors"
CAMERA_UPLOAD_DIR = UPLOAD_ROOT_DIR / "cameras"
MAP_UPLOAD_DIR = UPLOAD_ROOT_DIR / "maps"
    ...
    for metadata_path in sorted(MAP_UPLOAD_DIR.glob("**/metadata.json")):
        ...
        if not image_url:
            ...
            if image_path.exists():
                try:
                    image_url = f"/static/uploads/{image_path.relative_to(UPLOAD_ROOT_DIR).as_posix()}"
                except ValueError:
                    image_url = ""
        item = {
...
def static_url_for_path(path: Path) -> str:
    try:
        ...
        return f"/static/uploads/{quote(path.relative_to(UPLOAD_ROOT_DIR).as_posix(), safe='/')}"
