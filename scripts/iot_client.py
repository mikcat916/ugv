...
def load_config() -> Dict[str, Any]:
    ...
    parser.add_argument("--camera-device", default="", help="本地摄像头设备路径，如 /dev/video0")
    parser.add_argument("--camera-interval", type=int, default=0, help="摄像头快照上传间隔秒数，0 表示禁用")
    ...
    camera_device = args.camera_device or _get("camera_device")
    camera_interval_raw = args.camera_interval or int(_get("camera_interval", "0"))
    camera_interval = max(5, camera_interval_raw) if camera_interval_raw > 0 else 0
...
def _post_image(server: str, token: str, jpeg_data: bytes, channel: str, tls_verify: bool = True) -> Dict[str, Any]:
    """Upload JPEG image to /api/iot/camera/snapshot via multipart."""
    ...
    url = f"{server}/api/iot/camera/snapshot?channel={channel}"
    req = urllib.request.Request(
        url,
        data=body,
        headers={
...
def capture_camera_snapshot(device: str) -> Optional[bytes]:
    """
    Capture a JPEG snapshot from a local camera device.
    Tries fswebcam, then raspistill, then OpenCV.
    """
    # 方法1: fswebcam
    try:
        result = subprocess.run(
            ["fswebcam", "-d", device, "--no-banner", "--jpeg", "80", "-r", "640x480", "-"],
            capture_output=True, timeout=5, check=False,
        )
        if result.returncode == 0 and result.stdout[:2] == b"\xff\xd8":
            return result.stdout
    except (FileNotFoundError, subprocess.TimeoutExpired):
        pass

    # 方法2: raspistill (树莓派专有)
    try:
        result = subprocess.run(
            ["raspistill", "-n", "-w", "640", "-h", "480", "-q", "80", "-t", "1", "-o", "-"],
            capture_output=True, timeout=5, check=False,
        )
        if result.returncode == 0 and result.stdout[:2] == b"\xff\xd8":
            return result.stdout
...
def camera_upload_loop(server: str, token: str, device: str, interval: int, tls_verify: bool) -> None:
    """Background thread: capture and upload camera snapshots."""
    log.info(f"摄像头上传线程启动 | 设备: {device} | 间隔: {interval}s")
    while True:
        jpeg = capture_camera_snapshot(device)
        if jpeg:
            try:
                result = _post_image(server, token, jpeg, "mono", tls_verify=tls_verify)
                log.info(f"摄像头快照上传成功: {len(jpeg)} 字节, ok={result.get('ok')}")
            except RuntimeError as exc:
                log.error(f"摄像头快照上传失败: {exc}")
        else:
            log.warning(f"无法从 {device} 拍摄快照")
        time.sleep(interval)
...
def main() -> None:
    ...
    log.info(
        ...
        f" | 摄像头: {cfg['camera_device'] or '未配置'}"
    )
    if server.lower().startswith("https://") and not cfg["tls_verify"]:
        log.warning("HTTPS 证书校验已禁用，仅适用于受信任的内网调试环境")

    # 启动摄像头上传后台线程
    if cfg["camera_device"] and cfg["camera_interval"] > 0:
        import threading
        cam_thread = threading.Thread(
            target=camera_upload_loop,
            args=(server, token, cfg["camera_device"], cfg["camera_interval"], bool(cfg["tls_verify"])),
            daemon=True,
        )
        cam_thread.start()
