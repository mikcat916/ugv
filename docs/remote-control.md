...
- 遥测链路：`scripts/iot_client.py -> /api/iot/telemetry -> Web 看板`

## 关键文件

| 文件 | 作用 |
| --- | --- |
| `backend/main.py` | 提供控制接口、目标解析、TCP 转发和控制命令记录 |
| `backend/static/dashboard.js` | 渲染远程遥控页面，处理按住方向键持续发送指令 |
| `scripts/robot_control_server.py` | 运行在树莓派上的 TCP 控制服务 |
| `scripts/iot_client.py` | 运行在树莓派上的遥测和打卡上报客户端 |
| `scripts/bootstrap_iot_backend.py` | 初始化 IoT 表并生成设备 Token |
| `scripts/deploy_iot_client.py` | 从 Windows 远程部署 IoT 客户端到树莓派 |
| `backend/tests/test_robot_control_api.py` | 覆盖控制目标解析、权限、速度限幅和异常响应 |
...
树莓派侧脚本是 `scripts/robot_control_server.py`，核心职责如下：
...
远程控制页面位于 Web 后台的“设备与机器人 / 远程遥控无人车”相关入口，前端逻辑在 `backend/static/dashboard.js`。
