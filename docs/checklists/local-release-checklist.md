...
- [ ] 后端依赖：`python -m pip install -r apps/backend/requirements.txt`
...
- [ ] 运行 `python tools/dev/create_database.py`
- [ ] 如需清库重来，先运行 `mysql -u root -p robot_monitor < apps/backend/db/reset-db-dev.sql`
